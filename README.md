...
- Run the container shell with `nix run .#zisk-shell`. On first use this pulls
  the CI-published image from
  `ghcr.io/argumentcomputer/cargo-zisk:blake3-precompile` — no local image
  build needed. Pass `--cpu`/`--gpu` and `--key` to install the Zisk binaries
  and setup keys via `ziskup` inside the container (see `zisk-shell --help`)
...
`docker run -it ghcr.io/argumentcomputer/cargo-zisk:blake3-precompile`. CI
builds, tests, and publishes this image with Docker; the engines are
interchangeable.
